...
module.exports = function(RED) {
    function trafikantenFetcher(config) {
        ...
        this.on("input", function(msgArr) {
            ...
            var url = "http://reisapi.ruter.no/Favourites/GetFavourites?favouritesRequest=";
            var favouriteConfigs = [];
            for (var i = 0; i < msgArr.payload.length; i++) {
                var msg = msgArr.payload[i];
                if (msg.payload.config) {
                    favouriteConfigs.push(msg.payload.config);
                }
            }
            if (favouriteConfigs.length > 0) {
                _this.fetch(url + favouriteConfigs.join(","));
            } else {
                _this.send({});
            }
        });
